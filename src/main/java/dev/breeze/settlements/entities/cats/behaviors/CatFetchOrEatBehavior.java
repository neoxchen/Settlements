...
public final class CatFetchOrEatBehavior extends BaseCatBehavior {
    ...
    @Override
    protected boolean checkExtraStartConditionsRateLimited(@Nonnull ServerLevel level, @Nonnull Cat self) {
    ...
    private boolean scan(@Nonnull ServerLevel level, @Nonnull Cat self) {
        // Scan for nearby items
        Optional<List<ItemEntity>> itemMemory = self.getBrain().getMemory(CatMemoryType.NEARBY_ITEMS);
    ...
    @Override
    protected boolean canStillUse(@Nonnull ServerLevel level, @Nonnull Cat self, long gameTime) {
        // Check time limit
    ...
    @Override
    protected void start(@Nonnull ServerLevel level, @Nonnull Cat self, long gameTime) {
        super.start(level, self, gameTime);

        this.status = FetchStatus.SEEKING;
        if (self instanceof VillagerCat villagerCat) {
    ...
    @Override
    protected void tick(@Nonnull ServerLevel level, @Nonnull Cat self, long gameTime) {
        BaseVillager owner = this.getOwner(self);
        ...
        if (this.status == FetchStatus.SEEKING) {
            ...
            // Look at ground
            self.getLookControl().setLookAt(self.getX(), self.getY() - 1, self.getZ());
    ...
    @Override
    protected void stop(@Nonnull ServerLevel level, @Nonnull Cat self, long gameTime) {
